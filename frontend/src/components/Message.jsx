...
const ChatInterface = ({ loggedInUserId, selectedUserId }) => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await fetch(`${BACKEND_URL}/messages/${loggedInUserId}/${selectedUserId}`);
        if (response.ok) {
...
const markMessagesAsRead = async () => {
  try {
    await fetch(`${BACKEND_URL}/messages/markAsRead/${loggedInUserId}/${selectedUserId}`, {
      method: "PUT",
    });
  ...
  return (
    <div className="ml-64 flex flex-col h-screen">
      ...
      <div className="flex-1 p-4 bg-gray-100 overflow-y-auto">
        <div className="space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-center space-x-2 ${msg.sender === loggedInUserId ? "justify-end" : "justify-start"}`}>
              <div className={`p-3 rounded-lg shadow-md max-w-xs ${msg.sender === loggedInUserId ? "bg-blue-500 text-white" : "bg-white text-black"} ${msg.isUnread ? "border border-red-500 rounded-md p-1" : ""}`}
        >
                ...
                <a 
                  ...
                  className={`underline font-semibold ${msg.sender === loggedInUserId ? "text-right text-white" : "text-left text-black"}`}
                  >
      ...
      <div className="p-4 bg-white border-t flex items-center">
        ...
        />
        <button onClick={sendMessage} className="bg-blue-500 text-white px-4 py-2 rounded-md ml-2" disabled={!message.trim()}>
          Send
        </button>
      </div>
    </div>
